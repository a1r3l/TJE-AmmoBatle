...
Texture * TextureManager::getTexture(const char * textureName) {
	...
	std::string path = std::string("data/textures/")+textureName;
	std::map<std::string, Texture *>::iterator it;
	it = projectTextures.find(path);

	if (it != projectTextures.end()) {
		return it->second;
	}
	
	else {
		Texture * nTexture = new Texture();
		if(!nTexture->load(path.c_str())) std::cout<<"No se ha cargado la textura: "<< path << std::endl;
		projectTextures[path] = nTexture;
		return nTexture;
	}
}